...
function LegacySplatRedirect() {
  useEffect(() => {
    ...
    const rest = window.location.pathname.replace(/^\/fundraiser/, "");
    ...
    if (slug) {
      window.location.replace(tenantPath(slug, "fundraiser", rest) + window.location.search + window.location.hash);
