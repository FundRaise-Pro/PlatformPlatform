  ...
  const renderNav = () => (
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-xl border-b border-slate-100 px-10 py-5 flex justify-between items-center shrink-0">
      <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setStep('landing')}>
        <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center text-white shadow-xl group-hover:scale-110 transition duration-500">
           <Heart size={24} fill="currentColor" />
        </div>
        <span className="font-black text-xl tracking-tighter text-slate-900 uppercase">{config.tenantName}</span>
      </div>
      <div className="flex items-center gap-8">
        <button onClick={() => setStep('stories')} className={`text-[10px] font-black uppercase tracking-widest transition ${step === 'stories' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-900'}`}>Stories</button>
        <button onClick={() => setStep('partners')} className={`text-[10px] font-black uppercase tracking-widest transition ${step === 'partners' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-900'}`}>Partners</button>
        <button onClick={() => setStep('blog')} className={`text-[10px] font-black uppercase tracking-widest transition ${step === 'blog' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-900'}`}>Media</button>
        <button onClick={() => setStep('events')} className={`text-[10px] font-black uppercase tracking-widest transition ${step === 'events' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-900'}`}>Events</button>
        <button 
          onClick={() => handleStartDonate()}
          style={{ backgroundColor: config.primaryColor }}
          className="text-white px-8 py-3 rounded-full font-black text-[10px] shadow-xl hover:-translate-y-0.5 transition uppercase tracking-widest"
        >
  ...
  if (step === 'success') {
    return (
      <div className="flex-1 h-full bg-white flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-700 overflow-y-auto">
        ...
        <div className="w-full max-w-md bg-slate-50 border border-slate-100 rounded-[2.5rem] p-10 mb-10 text-left">
           <span className="inline-block px-3 py-1 bg-white border border-indigo-50 rounded-full text-[9px] font-black text-indigo-500 uppercase tracking-widest mb-4">Tax Verified</span>
           <h3 className="text-xl font-black text-slate-900 mb-6">Section 18A Certificate</h3>
           <button className="w-full py-4 bg-white border border-indigo-100 rounded-2xl text-[10px] font-black text-indigo-600 hover:bg-indigo-600 hover:text-white transition shadow-sm uppercase tracking-widest">
              <Download size={16} className="inline mr-2" /> Get PDF Receipt
           </button>
        </div>
        <button onClick={() => setStep('landing')} className="text-slate-300 font-black uppercase tracking-[0.2em] text-[10px] hover:text-indigo-600 transition">Back to Hub</button>
      </div>
    );
  }
      ...
      <div className="flex-1 h-full flex flex-col bg-white overflow-hidden animate-in fade-in duration-500">
        ...
        <div className="flex-1 overflow-y-auto p-12 custom-scrollbar bg-slate-50/30">
           <div className="max-w-6xl mx-auto">
              <h2 className="text-4xl font-black text-slate-900 mb-12 uppercase tracking-tighter">Impact Narratives</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {config.beneficiaryStories.map(story => (
                  <div key={story.id} className="bg-white rounded-[3rem] overflow-hidden shadow-2xl shadow-slate-200/50 border border-slate-100 group">
                     <div className="aspect-video overflow-hidden bg-slate-100">
                        <img src={story.image} className="w-full h-full object-cover group-hover:scale-110 transition duration-1000" alt={story.name} />
                     </div>
                     <div className="p-12">
                        <h3 className="text-2xl font-black text-slate-900 mb-4">{story.name}</h3>
                        <p className="text-slate-500 font-medium leading-relaxed italic mb-8">"{story.bio}"</p>
                        <div className="flex justify-between items-center pt-8 border-t border-slate-50">
                           <div>
                              <span className="block text-2xl font-black text-slate-900 tracking-tighter">${story.raised}</span>
                              <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Contributed</span>
                           </div>
                           <button className="px-8 py-3 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest">Story Details</button>
                        </div>
                     </div>
                  </div>
                ))}
           ...
           <div className="max-w-6xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {config.events.map(event => (
                  <div key={event.id} className="p-10 bg-white border border-slate-100 rounded-[3rem] shadow-xl flex flex-col hover:border-indigo-400 transition cursor-default">
                     ...
                     <div className="mt-auto pt-8 border-t border-slate-50 flex justify-between items-center">
                        ...
                        <button className="px-6 py-2 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg">Register</button>
                     </div>
                  </div>
                ))}
              </div>
           ...
           <div className="max-w-6xl mx-auto">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                 {config.blogPosts.map(post => (
                    <div key={post.id} className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 flex flex-col md:flex-row gap-8 items-center group">
                       <div className="w-full md:w-48 h-48 rounded-[2.5rem] overflow-hidden shrink-0 shadow-lg bg-slate-100">
                          <img src={post.image || 'https://images.unsplash.com/photo-1497215728101-856f4ea42174?auto=format&fit=crop&q=80&w=400'} className="w-full h-full object-cover group-hover:scale-110 transition duration-700" alt={post.title} />
                       </div>
                       <div className="flex-1">
                          ...
                          <h3 className="text-2xl font-black text-slate-900 mb-4 group-hover:text-indigo-600 transition">{post.title}</h3>
                          <p className="text-slate-500 text-xs font-medium italic mb-6 line-clamp-2">"{post.excerpt}"</p>
                          <button className="flex items-center gap-2 text-slate-900 font-black text-[9px] uppercase tracking-[0.2em] border-b-2 border-slate-100 pb-1 hover:border-indigo-600 transition">Read Article <ArrowRight size={14} /></button>
                       </div>
                    </div>
                 ))}
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-12 custom-scrollbar">
           <div className="max-w-6xl mx-auto">
              <header className="mb-24 text-center">
                 ...
                 <p className="text-slate-400 text-xl font-medium max-w-2xl mx-auto italic">Verified NPO compliance and high-impact CSR monitoring.</p>
              </header>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-20">
                {config.partnerTiers.map(tier => (
                  <div key={tier.id} className="p-12 bg-slate-50 rounded-[3rem] border-2 border-transparent hover:border-indigo-600 transition group shadow-lg">
                    ...
                    <button className="w-full py-5 bg-slate-900 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-widest group-hover:bg-indigo-600 transition shadow-xl">Inquire Partnership</button>
                  </div>
                ))}
              </div>
           </div>
      ...
      <div className="flex-1 h-full flex flex-col bg-white overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar selection:bg-indigo-100">
          <header className="relative pt-24 pb-32 px-10 max-w-7xl mx-auto flex flex-col items-center text-center">
             <span className="inline-block px-5 py-2 bg-indigo-50 border border-indigo-100 rounded-full text-[10px] font-black text-indigo-600 uppercase tracking-[0.3em] mb-12">Active {config.terminology.campaign}</span>
             ...
             <div className="w-full max-w-5xl bg-white border border-slate-100 shadow-[0_80px_120px_-30px_rgba(0,0,0,0.15)] rounded-[4rem] p-12 md:p-20 -mt-56 relative z-10">
                ...
                <div className="w-full h-8 bg-slate-50 rounded-full overflow-hidden mb-20 shadow-inner ring-1 ring-slate-100">
                   <div style={{ width: `${progressPercent}%`, backgroundColor: config.primaryColor }} className="h-full rounded-full transition-all duration-2000 ease-out shadow-lg" />
                </div>
          ...
          <section className="bg-slate-50/50 py-32 px-10">
             <div className="max-w-6xl mx-auto flex flex-col items-center">
                <span className="text-indigo-600 font-black text-[10px] uppercase tracking-[0.3em] mb-4">The Impact</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full mb-20">
                  {config.beneficiaryStories.slice(0, 2).map(story => (
                    <div key={story.id} className="bg-white rounded-[3rem] p-12 shadow-2xl shadow-slate-200/50 flex flex-col lg:flex-row gap-10 items-center border border-slate-100 group hover:border-indigo-400 transition cursor-pointer">
                       <img src={story.image} className="w-32 h-32 rounded-3xl object-cover grayscale group-hover:grayscale-0 transition duration-1000 shadow-lg" alt={story.name} />
                       <div className="text-center lg:text-left">
                          <h4 className="text-2xl font-black text-slate-900 mb-2">{story.name}</h4>
                          <p className="text-slate-500 font-medium italic leading-relaxed line-clamp-3">"{story.bio}"</p>
                       </div>
                    </div>
                  ))}
                </div>
                <button onClick={() => setStep('stories')} className="flex items-center gap-3 text-slate-900 font-black uppercase tracking-[0.2em] text-[10px] group transition">
             ...
             <div className="max-w-6xl mx-auto text-center">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
                   {config.partners.slice(0, 4).map(p => (
                     <div key={p.id} className="p-12 bg-slate-50 border border-slate-100 rounded-[3rem] flex flex-col items-center shadow-lg grayscale hover:grayscale-0 transition duration-700">
                ...
                <button onClick={() => setStep('partners')} className="mt-20 px-10 py-4 bg-slate-900 text-white rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 transition shadow-xl">Join the Ecosystem</button>
             </div>
          </section>
          ...
          <section className="py-32 px-10 bg-slate-900 text-white relative overflow-hidden">
             <div className="max-w-6xl mx-auto relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
                   ...
                   <button onClick={() => setStep('blog')} className="px-8 py-3 bg-white text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-100 transition">Explore Updates</button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                   {config.blogPosts.slice(0, 3).map(post => (
                      <div key={post.id} className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] hover:bg-white/10 transition group cursor-pointer">
                         ...
                         <h4 className="text-xl font-black mb-3 tracking-tight">{post.title}</h4>
             ...
             <div className="max-w-6xl mx-auto text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-20">
                   {config.events.slice(0, 2).map(event => (
                      <div key={event.id} className="p-12 bg-slate-50 rounded-[3.5rem] border border-slate-100 flex justify-between items-center text-left hover:border-indigo-500 transition cursor-pointer group shadow-lg">
                         <div className="flex items-center gap-8">
                            <div className="w-16 h-16 rounded-[1.5rem] bg-indigo-50 text-indigo-600 flex flex-col items-center justify-center font-black shadow-inner">
                            ...
                            <div>
                               <h4 className="text-2xl font-black text-slate-900 mb-1">{event.title}</h4>
                               <p className="text-slate-400 text-xs font-black uppercase tracking-widest flex items-center gap-2"><MapPin size={12} /> {event.venue}</p>
                            </div>
                         </div>
                         <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center text-slate-300 group-hover:bg-indigo-600 group-hover:text-white transition shadow-sm">
                            <ArrowRight size={24} />
                         </div>
                      </div>
                   ))}
                </div>
                <button onClick={() => setStep('events')} className="text-slate-300 font-black uppercase tracking-[0.2em] text-[10px] hover:text-indigo-600 transition underline underline-offset-8 decoration-indigo-200">Full Calendar Protocol</button>
             </div>
          </section>
